...
def create_subtask(task_id, title, assignee_name):
    """Создать подзадачу"""
    ...
    # Сначала найдем ID пользователя по имени
    ...
    for user in users_data.get('results', []):
        if user.get('name') == assignee_name:
            assignee_id = user.get('id')
            break
    
    if not assignee_id:
        print(f"❌ Пользователь {assignee_name} не найден")
        return None
...
def update_task_status(task_id, status):
    ...
    response = requests.patch(url, headers=headers, json=json.dumps(payload))
    if response.status_code != 200:
...
def main():
    ...
    subtask = create_subtask(task_id, subtask_title, "Arsentiy")
