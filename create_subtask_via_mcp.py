#!/usr/bin/env python3
"""
Создание подзадачи через MCP сервер
"""
...
async def find_task_and_create_subtask():
    """Найти задачу и создать подзадачу"""
    ...
    if not tasks_result:
        logger.error("❌ Задача 'Лого и иконка' не найдена")
        return
    ...
    if not tasks:
        logger.error("❌ Задача 'Лого и иконка' в статусе In Progress не найдена")
        return
...
async def main():
    """Основная функция"""
    try:
        await find_task_and_create_subtask()
