#!/usr/bin/env python3
"""
Массовое обновление обложек через Cloudflare Images
Прямая загрузка изображений и получение стабильных ссылок для Notion
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('cloudflare_cover_update.log', encoding='utf-8'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger(__name__)

class CloudflareCoverUpdater:
    def __init__(self):
    ...
    async def upload_to_cloudflare_images(self, image_data: bytes, filename: str) -> Optional[str]:
        """Загрузить изображение в Cloudflare Images"""
        try:
            if not self.cloudflare_account_id or not self.cloudflare_api_token:
                logger.error("Не настроены Cloudflare credentials")
                return None
            ...
            data = {
                ...
                'metadata': json.dumps({
                    'filename': filename,
                    'source': 'yandex_disk_cover'
                })
            }
            
            url = f"https://api.cloudflare.com/client/v4/accounts/{self.cloudflare_account_id}/images/v1"
                ...
                async with session.post(url, headers=headers, json=data) as response:
                    if response.status == 200:
                        ...
                        if result.get('success'):
                            ...
                            direct_url = image_info.get('variants', [None])[0] if image_info.get('variants') else None
                            if not direct_url:
                                # Формируем ссылку вручную
                                image_id = image_info.get('id')
                                direct_url = f"https://imagedelivery.net/{self.cloudflare_account_id}/{image_id}/public"
                            
                            logger.info(f"Загружено в Cloudflare Images: {direct_url}")
                            return direct_url
                        else:
                            logger.error(f"Ошибка Cloudflare API: {result.get('errors')}")
                            return None
                    else:
                        logger.error(f"Ошибка загрузки в Cloudflare: {response.status}")
                        return None
        except Exception as e:
    ...
    async def process_idea(self, idea: Dict) -> Tuple[bool, str]:
        ...
        try:
            ...
            # Загружаем в Cloudflare Images
            cloudflare_filename = f"cover_{idea['id'][:8]}.jpg"
            cover_url = await self.upload_to_cloudflare_images(image_data, cloudflare_filename)
            if not cover_url:
                self.stats['upload_failed'] += 1
                return False, "Не удалось загрузить в Cloudflare Images"
    ...
    async def run_analysis(self, limit: int = 10):
        ...
        logger.info(f"Начинаем массовое обновление обложек через Cloudflare Images (лимит: {limit})")
        
        if not self.cloudflare_account_id or not self.cloudflare_api_token:
            logger.error("Не настроены Cloudflare credentials (CLOUDFLARE_ACCOUNT_ID, CLOUDFLARE_API_TOKEN)")
            return
    ...
    def save_results(self):
        ...
        filename = f"cloudflare_cover_update_results_{timestamp}.json"
...
async def main():
    """Главная функция"""
    updater = CloudflareCoverUpdater()
