#!/usr/bin/env python3
"""
Поиск задач Arsentiy с правильным именем
...
def find_tasks_by_assignee(client: Client, db_id: str, assignee_name: str, status_filter: List[str] | None = None) -> List[Dict]:
    """Поиск задач по исполнителю в Tasks Database"""
    print(f"\n🔍 Поиск задач для '{assignee_name}' в Tasks Database...")
    
    # Правильные поля из DATA_STRUCTURE_GUIDE.md
    filter_conditions = {
        "property": "Участники",
        "people": {
            "contains": assignee_name
        }
    }
...
def find_subtasks_by_assignee(client: Client, db_id: str, assignee_name: str, status_filter: List[str] | None = None) -> List[Dict]:
    """Поиск подзадач по исполнителю в Subtasks Database"""
    print(f"\n🔍 Поиск подзадач для '{assignee_name}' в Subtasks Database...")
...
def main():
    """Основная функция"""
    print("🔍 ПОИСК ЗАДАЧ ARSENTIY (ПРАВИЛЬНОЕ ИМЯ)")
    print("=" * 50)
    ...
    subtasks = find_subtasks_by_assignee(
        client, 
