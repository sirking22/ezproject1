#!/usr/bin/env python3
"""
Массовое обновление обложек через публичные ссылки Яндекс.Диск
Парсинг HTML страниц для получения прямых ссылок на файлы
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('yandex_public_cover_update.log', encoding='utf-8'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger(__name__)

class YandexPublicCoverUpdater:
    def __init__(self):
    ...
    def extract_download_link_from_html(self, html_content: str) -> Optional[str]:
        ...
        try:
            ...
            # Ищем различные паттерны ссылок на файлы
            patterns = [
            ...
            for pattern in patterns:
                ...
                if matches:
                    ...
                    logger.info(f"Извлечена ссылка: {link}")
                    return link
            
            # Если не нашли по паттернам, ищем в мета-тегах
            meta_tags = soup.find_all('meta')
            for meta in meta_tags:
                if meta.get('property') == 'og:image' or meta.get('name') == 'twitter:image':
                    link = meta.get('content')
                    if link:
                        logger.info(f"Найдена ссылка в мета-теге: {link}")
                        return link
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(url, headers=headers, timeout=timeout) as response:
                    if response.status == 200:
                        ...
                        if 'image' in content_type or self.is_image_file(url):
                            return data, 'image'
                        elif 'video' in content_type or self.is_video_file(url):
                            return data, 'video'
                        else:
    ...
    async def run_analysis(self, limit: int = 10):
        ...
        logger.info(f"Начинаем массовое обновление обложек через публичные ссылки Яндекс.Диск (лимит: {limit})")
    ...
    def save_results(self):
        ...
        filename = f"yandex_public_cover_update_results_{timestamp}.json"
...
async def main():
    """Главная функция"""
    updater = YandexPublicCoverUpdater()
