#!/usr/bin/env python3
"""
Редактирование уже запущенного Blender через Python API
"""
...
def create_edit_script():
    """Создает скрипт для редактирования запущенного Blender"""
    script = '''
import bpy
import bmesh
import math
...
def send_to_running_blender():
    """Отправляем скрипт в уже запущенный Blender"""
    script_content = create_edit_script()
    
    # Создаем временный файл
    script_path = Path("temp_running_edit.py")
    with open(script_path, 'w', encoding='utf-8') as f:
        f.write(script_content)
    
    print(f"📝 Скрипт для запущенного Blender сохранен: {script_path.absolute()}")
    print("")
    print("🎯 Теперь выполните в УЖЕ ОТКРЫТОМ Blender:")
    print("")
    print("СПОСОБ 1 - Text Editor:")
    print(f"   1. Откройте Text Editor в Blender")
    print(f"   2. Нажмите 'Open' и выберите: {script_path.absolute()}")
    print(f"   3. Нажмите 'Run Script' (Alt+P)")
    print("")
    print("СПОСОБ 2 - Python Console:")
    print("   1. Откройте Python Console в Blender")
    print("   2. Скопируйте код ниже и вставьте:")
    print("=" * 60)
    print(script_content)
    print("=" * 60)
    print("")
    print("СПОСОБ 3 - Автоматическое выполнение:")
    print("   1. В Blender нажмите F4 (или выберите Window > Toggle System Console)")
    print("   2. В консоли введите: exec(open(r'{}').read())".format(script_path.absolute()))
    print("")
    print("✨ Выберите любой способ - результат будет одинаковым!")

if __name__ == "__main__":
    print("🔧 Редактирование запущенного Blender")
    print("=" * 50)
    
    send_to_running_blender()
    
    print("\n✨ Готово! Теперь твой запущенный Blender будет отредактирован!") 