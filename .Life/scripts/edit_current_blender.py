#!/usr/bin/env python3
"""
Редактирование текущего открытого Blender
"""
...
def create_edit_script():
    """Создает скрипт для редактирования текущей сцены"""
    script = '''
import bpy
import bmesh
import math
...
print("🔧 Редактирование текущей сцены...")
...
def run_edit_script():
    """Запускаем скрипт редактирования"""
    script_content = create_edit_script()
    
    # Создаем временный файл
    script_path = Path("temp_edit_script.py")
    with open(script_path, 'w', encoding='utf-8') as f:
        f.write(script_content)
    
    print(f"📝 Скрипт редактирования сохранен: {script_path.absolute()}")
    print("")
    print("🎯 Теперь выполните в Blender:")
    print(f"   1. Откройте Text Editor")
...
if __name__ == "__main__":
    print("🔧 Редактирование текущего Blender")
    print("=" * 50)
    
    run_edit_script()
    
    print("\n✨ Готово! Теперь твоя сцена станет еще лучше!") 